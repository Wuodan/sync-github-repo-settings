...
async function main() {
  ...
  try {
    ...
    for (let index = startIndex; index < manifestEntries.length; index += 1) {
      ...
      await retry(`copy ${entry.digest}`, _COPY_RETRY_COUNT, _COPY_RETRY_DELAY_MS, async () => {
        runCraneCopy(config.craneBin, sourcePackage.owner, sourcePackage.packageName, config.targetOwner, config.targetPackage, entry.digest);
      });
